...
class TestProxies:
    ...
    def test_proxy(self, proxy):
        ...
        try:
            ...
            with self.lock:
                self.worked += 1
                self.worked_proxy.append(proxy)
        except:
            with self.lock:
                self.failed += 1
        finally:
    ...
    def examine(self):
        ...
        if worked == 0:
            print('This is a bad proxy list \n')
        elif (failed - worked) >= 0.1:
            print('This is a bad proxy list \n')
        elif (failed - worked) == 0:
            print('This is a bad proxy list\n')
        else:
            print('This is a good proxy list\n')

        print('\nWorked proxies:')
        for proxy in self.worked_proxy:
            print(proxy['ip']+':'+str(proxy['port']))

#add site you want to test here
